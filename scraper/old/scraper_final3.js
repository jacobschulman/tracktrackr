...
// ── config ─────────────────────────────────────────────────────────
// Resolve index file — checks script dir, ~/git/tracktrackr-scraper, then ~/Downloads
function resolveIndexPath(filename) {
  const candidates = [
    path.join(__dirname, filename),
    path.join(os.homedir(), 'git', 'tracktrackr-scraper', filename),
    path.join(os.homedir(), 'Downloads', filename),
  ];
  for (const p of candidates) {
    if (fs.existsSync(p)) { mLog(`Index found: ${p.replace(os.homedir(), '~')}`); return p; }
  }
  return candidates[0]; // will fail with clear error
}
const INDEX_FILE = resolveIndexPath(ARGS.index || 'tracklore_index_complete_1467sets.json');
